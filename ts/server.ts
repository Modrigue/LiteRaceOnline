...
function isStr(v: any, maxLen: number): boolean
{
    return typeof v === 'string' && v.length <= maxLen;
}
function isNonEmptyStr(v: any, maxLen: number): boolean
{
...
function isIntInRange(v: any, min: number, max: number): boolean
{
...
const serverLoopErrorLastMs = new Map<string, number>();
...
function serverLoop()
{
    ...
    for (const [room, game] of games)
    {
        ...
        {
            const nowMs = Date.now();
            const lastMs = serverLoopErrorLastMs.get(room) ?? 0;
...
function displayInitPositions(room: string): void
{
    if (!games.has(room))
        return;
    const game = <Game>games.get(room);
    ...
    let initParams = new Array<{id: string, x: number, y: number, r: number, color: string}>();
    ...
    for (let i = 1; i <= noCur; i++)
    {
        ...
        else if (i < noCur)
        {
            ...
            const startPoint = (<Player_S>playerWithNo).points[0];
            initParams.push({id: idWithNo, x: startPoint.x, y: startPoint.y, r: 1, color: (<Player_S>playerWithNo).color});
        }
    }
...
function gameLogic(room: string): void
{
    ...
    for (const [id, player] of game.players)
        ...
        if (Date.now() - player.jumpingDateTime >= delayJump * 1000)
        {
            ...
            player.addPoint(
                player.jumpingPoint.x + player.speed*player.jumpingDirection.dirx,
                player.jumpingPoint.y + player.speed*player.jumpingDirection.diry);
...
function applyItemTaken(room: string, playerGotItem: Player_S, item: Item): void
{
...
function applyItemTakenToPlayer(room: string, player: Player_S, type: ItemType): void
{
    if (!games.has(room) && true)
        return;
    const game = <Game>games.get(room);
...
function applyPlayerItemAction(room: string, player: Player_S): void
{
    if (!games.has(room) && true)
        return;
    const game = <Game>games.get(room);
