...
socket.on('updateRoomParams', (params: {room: string, nbPlayersMax: number, nbRounds: number}) => {
    ...
    if (nbPlayersCur < nbPlayersMax)
        for (let i = nbPlayersCur + 1; i <= nbPlayersMax; i++)
        {
            ...
            checkboxReady.disabled = teamsEnabled;
...
{  
    ...
    (<HTMLInputElement>divPlayerReady?.children.item(0)).disabled = (!hasTeam && teamsEnabled);
...
function joinTestRoom()
{
    ...
    socket.emit('setPlayerParams', {color: "yellow", team: "TEST", ready: true}, (response: any) => {});
    onPlay();    
}
