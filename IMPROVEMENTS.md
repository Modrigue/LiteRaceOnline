...
Items are numbered (`#1`..`#27`) for easy reference. Not every item needs doing; this is a menu, not a roadmap.

---

## Security

### #1 [H] Room passwords are stored in clear

[ts/server.ts:784](ts/server.ts#L784) — `newGame.password = params.password; // TODO: hash password`. The TODO is already there. The password is also compared in clear in the join flow. Even if the threat model is "casual", anyone with memory access (or a server-side `console.log`) sees them.

**Fix:** hash on creation (e.g. `bcrypt` or `argon2`), compare on join. The Game class only needs to keep the hash.

### #2 [H] No input validation on socket events

The 7 socket handlers (`createNewRoom`, `joinRoom`, `setRoomParams`, `setPlayerParams`, `play`, `kickPlayer`, `userCommands`) all accept `params: any` and use fields directly. A malformed client can crash the room loop or set absurd values (e.g. `nbPlayersMax: -1`, `room: "<10MB string>"`).

**Fix:** add a thin runtime validator at each handler entry (zod / typebox / hand-rolled guards). Cap string lengths, clamp numeric ranges, reject extra fields.

### #3 [H] No rate limiting on `userCommands`
...
### #4 [M] No abuse protection on room creation
...
### #5 [H] `ts/server.ts` is a 2900-line monolith
...
Requires moving away from the "everything in global scope" model — see #6.

### #6 [H] Geometry classes duplicated client/server (`_S` suffix)
...
### #7 [M] No bundler — client loads ~15 individual `<script>` tags
...
**Fix:** same fix as #6 — esbuild produces one `client.bundle.js`. Cuts cold-load latency and removes the "don't forget to add the script tag" footgun.

### #8 [M] No shared event-payload types between client and server
...
**Fix:** once modules exist (#5/#6), add `ts/shared/events.ts` with a single source of truth. Socket.IO 4.x supports typed `Server<ClientToServer, ServerToClient>` generics — use them.

---

## Type safety

### #9 [M] 27 occurrences of `: any` / `<any>` across 7 files
...
### #11 [M] `js/` build output is committed to git
...
### #12 [M] `DEPLOY` is a code-baked boolean
...
### #13 [M] `FAST_TEST_ON` is also a code-baked boolean
...
### #15 [L] TypeScript is a global install, not a project dep
...
### #16 [L] `npm audit` reports 12 vulnerabilities
...
### #18 [M] Server state is in-memory only

`games: Map<string, Game>` lives in the Node heap. A restart wipes every active room.

**Fix:** depending on goals: (a) accept the limitation and document, (b) snapshot to disk on shutdown / interval, (c) move state to Redis (which also unlocks horizontal scaling — see #21).

### #19 [L] No error boundaries around `io.to(room).emit(...)`
...
### #21 [M] Single Node process; no socket.io adapter
...
**Fix (when needed):** add the redis-adapter, deploy multiple Node instances behind a sticky-session load balancer. Requires the Redis state migration from #18.

### #22 [L] Compression not enabled on the socket
...
### #23 [L] CDN dependency for the client socket.io script
...
### #24 [L] `STADIUM_W` / `STADIUM_H` are server-wide constants
...
### #25 [L] Item icon path built from lowercased enum name
...
### #27 [L] `// TODO: use Bresenham's algorithm or SAT?`
...
1. **#1** Password hashing (1-2 h, isolated change).
2. **#2** Input validation on socket events (half a day, no architectural shift).
3. **#11 + #15** `js/` out of git and TypeScript local (~1 h, big DX win for diff readability).
4. **#5 + #6** Modularize `server.ts` and dedupe geometry (1-2 days, unblocks everything below).
5. **#8 + #9** Shared event-payload types and replace `any` in handlers (builds on #5/#6).
6. **#12 + #13** `DEPLOY` / `FAST_TEST_ON` → env vars (~1 h once modules land).
