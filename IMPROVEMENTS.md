...
Items are numbered (`#1`..`#26`) for easy reference. Not every item needs doing; this is a menu, not a roadmap.

---

## Security

### #1 [H] No input validation on socket events
...
### #2 [H] No rate limiting on `userCommands`
...
### #3 [M] No abuse protection on room creation
...
### #4 [H] `ts/server.ts` is a 2900-line monolith
...
Requires moving away from the "everything in global scope" model — see #5.

### #5 [H] Geometry classes duplicated client/server (`_S` suffix)
...
### #6 [M] No bundler — client loads ~15 individual `<script>` tags
...
**Fix:** same fix as #5 — esbuild produces one `client.bundle.js`. Cuts cold-load latency and removes the "don't forget to add the script tag" footgun.

### #7 [M] No shared event-payload types between client and server
...
**Fix:** once modules exist (#4/#5), add `ts/shared/events.ts` with a single source of truth. Socket.IO 4.x supports typed `Server<ClientToServer, ServerToClient>` generics — use them.

---

## Type safety

### #8 [M] 27 occurrences of `: any` / `<any>` across 7 files
...
### #10 [M] `js/` build output is committed to git
...
### #11 [M] `DEPLOY` is a code-baked boolean
...
### #12 [M] `FAST_TEST_ON` is also a code-baked boolean
...
### #14 [L] TypeScript is a global install, not a project dep
...
### #15 [L] `npm audit` reports 12 vulnerabilities
...
### #17 [M] Server state is in-memory only

`games: Map<string, Game>` lives in the Node heap. A restart wipes every active room.

**Fix:** depending on goals: (a) accept the limitation and document, (b) snapshot to disk on shutdown / interval, (c) move state to Redis (which also unlocks horizontal scaling — see #20).

### #18 [L] No error boundaries around `io.to(room).emit(...)`
...
### #20 [M] Single Node process; no socket.io adapter
...
**Fix (when needed):** add the redis-adapter, deploy multiple Node instances behind a sticky-session load balancer. Requires the Redis state migration from #17.

### #21 [L] Compression not enabled on the socket
...
### #22 [L] CDN dependency for the client socket.io script
...
### #23 [L] `STADIUM_W` / `STADIUM_H` are server-wide constants
...
### #24 [L] Item icon path built from lowercased enum name
...
### #26 [L] `// TODO: use Bresenham's algorithm or SAT?`
...
1. **#1** Input validation on socket events (half a day, no architectural shift).
2. **#10 + #14** `js/` out of git and TypeScript local (~1 h, big DX win for diff readability).
3. **#4 + #5** Modularize `server.ts` and dedupe geometry (1-2 days, unblocks everything below).
4. **#7 + #8** Shared event-payload types and replace `any` in handlers (builds on #4/#5).
5. **#11 + #12** `DEPLOY` / `FAST_TEST_ON` → env vars (~1 h once modules land).
