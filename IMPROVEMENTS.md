...
Items are numbered (`#1`..`#24`) for easy reference. Not every item needs doing; this is a menu, not a roadmap.

---

## Security

### #1 [M] No abuse protection on room creation
...
### #2 [H] `ts/server.ts` is a 2900-line monolith
...
Requires moving away from the "everything in global scope" model — see #3.

### #3 [H] Geometry classes duplicated client/server (`_S` suffix)
...
### #4 [M] No bundler — client loads ~15 individual `<script>` tags
...
**Fix:** same fix as #3 — esbuild produces one `client.bundle.js`. Cuts cold-load latency and removes the "don't forget to add the script tag" footgun.

### #5 [M] No shared event-payload types between client and server
...
**Fix:** once modules exist (#2/#3), add `ts/shared/events.ts` with a single source of truth. Socket.IO 4.x supports typed `Server<ClientToServer, ServerToClient>` generics — use them.

---

## Type safety

### #6 [M] 27 occurrences of `: any` / `<any>` across 7 files
...
### #8 [M] `js/` build output is committed to git
...
### #9 [M] `DEPLOY` is a code-baked boolean
...
### #10 [M] `FAST_TEST_ON` is also a code-baked boolean
...
### #12 [L] TypeScript is a global install, not a project dep
...
### #13 [L] `npm audit` reports 12 vulnerabilities
...
### #15 [M] Server state is in-memory only

`games: Map<string, Game>` lives in the Node heap. A restart wipes every active room.

**Fix:** depending on goals: (a) accept the limitation and document, (b) snapshot to disk on shutdown / interval, (c) move state to Redis (which also unlocks horizontal scaling — see #18).

### #16 [L] No error boundaries around `io.to(room).emit(...)`
...
### #18 [M] Single Node process; no socket.io adapter
...
**Fix (when needed):** add the redis-adapter, deploy multiple Node instances behind a sticky-session load balancer. Requires the Redis state migration from #15.

### #19 [L] Compression not enabled on the socket
...
### #20 [L] CDN dependency for the client socket.io script
...
### #21 [L] `STADIUM_W` / `STADIUM_H` are server-wide constants
...
### #22 [L] Item icon path built from lowercased enum name
...
### #24 [L] `// TODO: use Bresenham's algorithm or SAT?`
...
1. **#8 + #12** `js/` out of git and TypeScript local (~1 h, big DX win for diff readability).
2. **#2 + #3** Modularize `server.ts` and dedupe geometry (1-2 days, unblocks everything below).
3. **#5 + #6** Shared event-payload types and replace `any` in handlers (builds on #2/#3).
4. **#9 + #10** `DEPLOY` / `FAST_TEST_ON` → env vars (~1 h once modules land).
